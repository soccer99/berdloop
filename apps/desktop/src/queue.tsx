...
function Log({
  ...
  //
  // A tool line is exempt: its text is a tool's name and the arguments it was
  // called with, not prose an agent wrote, so it is drawn as it came.
  const shown = messages
    .map((message) => ({
      ...message,
      text:
        message.role === "tool" ? message.text : stripDiffBodies(message.text),
      files: message.role === "tool" ? [] : diffFiles(message.text),
    }))
